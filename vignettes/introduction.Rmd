...
Before looking at the predictions, it's a good idea to check the quality of the spline fits. If necessary, fitting parameters can be adjusted using the `fitMeanArgs` argument, which get passed to `bigspline`.
```{r}
...
Here we'll run cross-validation over three values of `sumabsv` and four values of `nSPC`. Typically a few values of `sumabsv` will suffice, and `nSPC` in this example can only range from 1 to 10 (because 10 is the default value of `nTime`). `spcResultList` and `predResultList` are actually nested lists, first by values of `sumabsv`, then by folds.
```{r}
